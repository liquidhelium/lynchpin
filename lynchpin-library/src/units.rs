...
fn share_fr(total_fr: Fr, fr: Fr, remaining: Col) -> Col {
    if total_fr == Fr::zero() || remaining <= 0 {
        return 0;
    }
    // Avoid overflow: compute as f64 then round.
    let share = remaining as f64 * (fr.get() as f64 / total_fr.get() as f64);
    share.round() as Col
}

/// Shrink auto-sized columns fairly so that total fits in `available`.
fn shrink_auto(cols: &mut [Col], available: Col) {
    let count = cols.iter().filter(|&&c| c > 0).count();
    if count == 0 {
        return;
    }

    let mut last;
    let mut fair = Col::MIN;
    ...
    while changed && overlarge > 0 {
        changed = false;
        last = fair;
        fair = redistribute / overlarge as Col;

        for &c in cols.iter() {
            if c > 0 && c <= fair && c > last {
                redistribute -= c;
                overlarge -= 1;
                changed = true;
            }
...
///
/// This replicates the paged column-sizing algorithm:
...
/// `Auto` track (0 for non-auto tracks).  Callers are responsible for
/// measuring cell content beforehand.
///
...
pub fn resolve_tracks(
    ...
    let mut auto_total: Col = 0;
    for (i, sizing) in tracks.0.iter().enumerate() {
        if *sizing == Sizing::Auto {
            let c = auto_sizes.get(i).copied().unwrap_or(1).max(1);
            resolved[i] = c;
            auto_total += c;
        }
    }
    ...
    if leftover > 0 && total_fr != Fr::zero() {
...
pub fn spacing_to_cols(spacing: &typst::layout::Spacing, styles: StyleChain) -> Col {
    match spacing {
        ...
        typst::layout::Spacing::Fr(_) => 1, // fractional spacing → 1 col minimum
    }
}
...
pub fn spacing_to_rows(spacing: &typst::layout::Spacing, styles: StyleChain) -> Row {
    match spacing {
        typst::layout::Spacing::Rel(rel) => rel_to_cols(rel, styles) as Row,
        typst::layout::Spacing::Fr(_) => 1,
    }
}

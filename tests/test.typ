...
//   高亮、上标/下标、智能引号、链接、引用块、行内代码、嵌套样式、
//   H/V 间距、Box 容器、Block 容器、分页符、文字大小写变换
//
// ⚠️  已知实现缺陷（通过本测试文件发现）：
//   1. BoxElem 曾有无限递归 bug（pack_ref 返回自身）→ 已修复
//   2. ListElem / EnumElem / TermsElem：经 realize(Target::Paged) 后
//      children 已被展开为块，原始 elem 的 children 为空，导致无输出
//   3. RawElem 代码块：经 realize 后每行被展开为 `line` elem，
//      当前不处理 line，导致代码块内容全部丢失
// ============================================================

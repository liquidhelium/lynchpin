//! Rowspan handling: measurement, simulation, and layout.
//!
//! Mirrors `lynchpin-layout/src/grid/rowspans.rs`.
//! Handles cells that span multiple rows, including unbreakable row groups,
//! auto row measurement with rowspans, and rowspan simulation.

use typst_library::diag::SourceResult;
use typst_library::engine::Engine;
use typst_library::foundations::{Resolve, StyleChain};
use typst_library::layout::grid::resolve::{Cell, Repeatable};
use typst_library::layout::{Abs, Axes, Fr, Length, Rel, Sizing};

use lynchpin_library::*;

use crate::grid::layout_cell;
use super::layouter::{GridLayouter, Row, points};
...
pub struct Rowspan {
    ...
    /// See [`RowState::is_being_repeated`].
...
pub struct CellMeasurementData<'layouter> {
    ...
    pub backlog: Option<&'layouter [TermScalar]>,
...
// ── TermScalar extensions (mirror Abs methods used in paged grid) ────────────
...
fn resolve_rel(rel: &Rel<Length>, base: TermScalar, styles: StyleChain) -> TermScalar {
    let base_abs = Abs::pt(base.get() as f64);
    let result_abs: Abs = rel.resolve(styles).relative_to(base_abs);
...
impl GridLayouter<'_> {
    ...
    pub fn layout_rowspan(
        ...
        current_region_data: Option<(&mut TermFrame, TermScalar)>,
        engine: &mut Engine,
    ) -> SourceResult<()> {
        let Rowspan {
            x,
        ...
        // Prepare regions.
        let size = TermSize::new(width, *first_height);
        ...
        let fragment = layout_cell(cell, engine, locator, self.styles, pod, is_being_repeated)?;
        ...
        let finished_header_rows = self
            ...
            .chain(std::iter::repeat(TermScalar::ZERO));
            ...
            .zip(fragment)
        {
            ...
            finished.push_frame(TermPoint::new(dx, dy), frame);
        }
    ...
    pub fn check_for_unbreakable_rows(
        ...
        if self.unbreakable_rows_left == 0 {
            ...
            while !self.regions.size.rows.fits(row_group.height) && self.may_progress_with_repeats()
            {
        ...
        if self.unbreakable_rows_left > 1 {
            ...
            for rowspan_data in self
                .rowspans
                .iter_mut()
                .filter(|rowspan| rowspan.y == current_row)
            {
    ...
    pub fn simulate_unbreakable_row_group(
        ...
        for (y, row) in self.grid.rows.iter().enumerate().skip(first_row) {
            if amount_unbreakable_rows.is_none() {
                ...
                unbreakable_rows_left = unbreakable_rows_left.max(additional_unbreakable_rows);
            }
            ...
            let height = match row {
                Sizing::Rel(v) => resolve_rel(v, regions.base().rows, self.styles),
                ...
                Sizing::Fr(_) => TermScalar::ZERO,
            };
            row_group.height = row_group.height + height;
    ...
    pub fn prepare_auto_row_cell_measurement(
        ...
        let mut custom_backlog: Vec<TermScalar> = vec![];

        // This closure is used to subtract the expected header and footer
        ...
        let mut subtract_header_footer_height_from_regions = || {
                    ...
                    || matches!(&self.grid.footer, Some(footer) if footer.repeated))
            {
                ...
                let mapped_regions = self.regions.map(|size| {
                    TermSize::new(
                        size.cols,
                        size.rows
                ...
                custom_backlog = mapped_regions.backlog.clone();
            ...
            (Some(self.regions.backlog.as_slice()), self.regions.last)
        };
        ...
        if rowspan == 1 {
            ...
            height_in_this_region = TermScalar::ZERO;
            frames_in_previous_regions = 0;
        } else {
            // Height of the rowspan covered by spanned rows in the current
            // region.
            let laid_out_height: TermScalar = self
                .current
                .lrows
                .iter()
                .filter_map(|row| match row {
                    Row::Frame(frame, y, _) if (parent.y..parent.y + rowspan).contains(y) => {
                        Some(frame.height())
                    }
            ...
            let unbreakable_height: TermScalar = row_group_data
                .into_iter()
            ...
            if let Some((rowspan_full, [rowspan_height, rowspan_other_heights @ ..])) = self
                .rowspans
                .iter()
                ...
                .map(|data| (data.region_full, &*data.heights))
            {
                ...
                last = self.regions.last.map(|size| {
                    size - self.current.repeating_header_height - self.current.footer_height
                });
            } else {
                ...
                height = if breakable {
                    height_in_this_region + self.regions.size.rows
                } else {
                    TermScalar::INFINITY
                };
                (backlog, last) = subtract_header_footer_height_from_regions();
    ...
    pub fn prepare_rowspan_sizes(
        ...
        if let Some(first_frame_size) = sizes.first_mut() {
            ...
            *first_frame_size =
                (*first_frame_size - measurement_data.height_in_this_region).max(TermScalar::ZERO);
        }
        ...
        // row.
        let will_be_covered_height: TermScalar = self
            .grid
            .rows
            .iter()
            .skip(auto_row_y + 1)
            ...
            .map(|row| match row {
                Sizing::Rel(v) => resolve_rel(v, self.regions.base().rows, self.styles),
                _ => TermScalar::ZERO,
            })
            .sum();
    ...
    pub fn simulate_and_measure_rowspans_in_auto_row(
        &self,
        y: usize,
        resolved: &mut Vec<TermScalar>,
        pending_rowspans: &[(usize, usize, Vec<TermScalar>)],
        unbreakable_rows_left: usize,
        row_group_data: Option<&UnbreakableRowGroup>,
        ...
        let mut simulated_sizes: Vec<TermScalar> = vec![];
        ...
        for (parent_y, rowspan, sizes) in pending_rowspans {
            ...
            for (simulated_target, rowspan_size) in simulated_sizes.iter_mut().zip(&mut sizes) {
        ...
        let mut simulated_regions = self.regions.clone();
        simulated_regions.size.rows -=
            row_group_data.map_or(TermScalar::ZERO, |row_group| row_group.height);
        ...
        if let Some(original_last_resolved_size) = last_resolved_size {
            ...
            simulated_regions.size.rows -= original_last_resolved_size;
        }
    ...
    fn run_rowspan_simulation(
        ...
        mut simulated_regions: TermRegions,
        simulated_sizes: &mut Vec<TermScalar>,
        engine: &mut Engine,
        last_resolved_size: Option<TermScalar>,
        unbreakable_rows_left: usize,
        mut disambiguator: usize,
    ) -> SourceResult<bool> {
        let requested_rowspan_height =
            simulated_sizes.iter().sum::<TermScalar>() - last_resolved_size.unwrap_or_default();

        let mut amount_to_grow = TermScalar::ZERO;

        for _attempt in 0..5 {
            let rowspan_simulator =
                RowspanSimulator::new(disambiguator, simulated_regions.clone(), &self.current);
            ...
            while extra_amount_to_grow > TermScalar::ZERO
                && simulated_regions.size.rows < extra_amount_to_grow
            {
                extra_amount_to_grow -=
                    simulated_regions.size.rows.max(TermScalar::ZERO);
                simulated_regions.next();
                simulated_regions.size.rows -=
                    self.current.repeating_header_height + self.current.footer_height;
                disambiguator += 1;
...
impl RowspanSimulator {
    ...
    fn new(
        ...
        Self {
            ...
            total_spanned_height: TermScalar::ZERO,
            latest_spanned_gutter_height: TermScalar::ZERO,
        }
    }
    ...
    fn simulate_rowspan_layout(
        ...
        y: usize,
        max_spanned_row: usize,
        amount_to_grow: TermScalar,
        requested_rowspan_height: TermScalar,
        mut unbreakable_rows_left: usize,
        layouter: &GridLayouter<'_>,
        engine: &mut Engine,
    ) -> SourceResult<TermScalar> {
        ...
        for (offset, row) in spanned_rows.iter().enumerate() {
            ...
            if unbreakable_rows_left == 0 {
                ...
                while !self.regions.size.rows.fits(row_group.height)
            ...
            match row {
                Sizing::Rel(v) => {
                    let height = resolve_rel(v, self.regions.base().rows, layouter.styles);
                    self.total_spanned_height = self.total_spanned_height + height;
                    if is_gutter {
                ...
                Sizing::Fr(_) if is_gutter => {
                    self.latest_spanned_gutter_height = TermScalar::ZERO;
                }
                Sizing::Fr(_) => {}
            }
    ...
    fn simulate_header_footer_layout(
        ...
        let (repeating_headers, header_height) =
            if !layouter.repeating_headers.is_empty() || !layouter.pending_headers.is_empty() {
                ...
                (None, TermScalar::ZERO)
            };

        let footer_height = if let Some(footer) = layouter
            .grid
            .footer
            .as_ref()
        ...
        while !self
            ...
            .size
            .rows
            .fits(header_height + footer_height)
        ...
        self.regions.size.rows -= self.header_height + self.footer_height;
        self.initial_after_repeats = self.regions.size.rows;
    ...
    fn may_progress_with_repeats(&self) -> bool {
        self.could_progress_at_top
            || self.regions.last.is_some()
                && self.regions.size.rows != self.initial_after_repeats
    }
}

/// Subtracts some size from the end of a vector of sizes.
/// For example, subtracting 5pt from \[2pt, 1pt, 3pt\] will result in \[1pt\].
fn subtract_end_sizes(sizes: &mut Vec<TermScalar>, mut subtract: TermScalar) {
    while subtract > TermScalar::ZERO
        && sizes.last().is_some_and(|&size| size <= subtract)
    {
        subtract = subtract - sizes.pop().unwrap();
    }
    if subtract > TermScalar::ZERO
        && let Some(last_size) = sizes.last_mut()
    {
        *last_size = *last_size - subtract;
    }
}

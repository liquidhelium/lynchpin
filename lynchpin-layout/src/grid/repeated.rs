//! Header/footer repetition and orphan prevention.
//!
//! Mirrors `lynchpin-layout/src/grid/repeated.rs`.
//! Handles placing, repeating, and simulating headers and footers across
//! region breaks, including orphan prevention and short-lived headers.
...
impl<'a> GridLayouter<'a> {
    ...
    pub fn may_progress_with_repeats(&self) -> bool {
        ...
        self.current.could_progress_at_top
            || self.regions.last.is_some()
                && self.regions.size.rows != self.current.initial_after_repeats
    }
    ...
    pub fn place_new_headers(
        ...
        *consecutive_header_count += 1;
        ...
        if new_upcoming_headers.first().is_some_and(|next_header| {
            consecutive_headers.last().is_none_or(|latest_header| {
                !latest_header.short_lived && next_header.range.start == latest_header.range.end
        ...
        *consecutive_header_count = 0;
        ...
        let first_conflicting_pos = self
            .repeating_headers
            .partition_point(|h| h.level < first_level);
        ...
        for removed_height in self
            .current
            .repeating_header_heights
            .drain(first_conflicting_pos..)
        {
        ...
        if consecutive_headers.last().is_some_and(|h| h.short_lived) {
            ...
            let snapshot_created = self.layout_new_headers(consecutive_headers, false, engine)?;
    ...
    fn layout_header_rows(
        ...
        let mut header_height = TermScalar::ZERO;
            ...
            header_height = header_height
                + self
                    .layout_row_with_state(
                        ...
                        disambiguator,
                        RowState {
                            current_row_height: Some(TermScalar::ZERO),
    ...
    pub fn layout_active_headers(&mut self, engine: &mut Engine) -> SourceResult<()> {
        ...
        while self.unbreakable_rows_left == 0
            && !self.regions.size.rows.fits(header_height)
            && self.may_progress_with_repeats()
        {
            // Advance regions without any output until we can place the
            // header and the footer.
            self.finish_region_internal(
                TermFrame::soft(TermSize::new(TermScalar::ZERO, TermScalar::ZERO)),
                vec![],
                Default::default(),
            );
        ...
        self.current.last_repeated_header_end = self
            .repeating_headers
            .last()
            .map(|h| h.range.end)
            .unwrap_or_default();
    ...
    pub fn layout_new_headers(
        ...
        let mut at_top = self.regions.size.rows == self.current.initial_after_repeats;
    ...
    pub fn simulate_header_height<'h: 'a>(
        ...
        let mut height = TermScalar::ZERO;
        for header in headers {
            height = height
                + self
                    .simulate_header(header, regions, engine, disambiguator)?
                    .height;
        }
        Ok(height)
    }
    ...
    pub fn prepare_footer(
        ...
        let mut skipped_region = false;
        while self.unbreakable_rows_left == 0
            && !self.regions.size.rows.fits(footer_height)
...
#[inline]
pub fn total_header_row_count<'h>(headers: impl IntoIterator<Item = &'h Header>) -> usize {
    headers
        .into_iter()
        .map(|h| h.range.end - h.range.start)
        .sum()
}

...
const FIGURE_CAPTION_RULE: ShowFn<FigureCaption> = |elem, engine, styles| {
    ...
    let _page_width = lynchpin_library::resolve_page_size(styles).cols;
    Ok(TermBlockElem::new()
        .with_body(Some(TermBlockBody::SingleLayouter(TermBlockCallback::new(
            elem.clone(),
...
const FOOTNOTE_ENTRY_RULE: ShowFn<FootnoteEntry> = |elem, engine, styles| {
    ...
    let _pw = lynchpin_library::resolve_page_size(styles).cols;
    Ok(TermBlockElem::new()
        .with_body(Some(TermBlockBody::SingleLayouter(TermBlockCallback::new(
            elem.clone(),
            move |_elem, eng, locator, st, region| {
    ...
    Ok(TermBlockElem::new()
        .with_body(Some(TermBlockBody::SingleLayouter(TermBlockCallback::new(
            ...
            |elem, engine, locator, styles, region| {
                ...
                let frames = lynchpin_library::fragment_into_frames(fragment);
...
const LAYOUT_RULE: ShowFn<LayoutElem> = |elem, _, _| {
    Ok(TermBlockElem::new()
        .with_body(Some(TermBlockBody::SingleLayouter(TermBlockCallback::new(
            ...
            |elem, engine, locator, styles, region| {
                let width_i64: i64 =
                    lynchpin_library::resolve_page_size(styles).cols.get() as i64;
                let height_i64: i64 =
                    lynchpin_library::resolve_page_size(styles).rows.get() as i64;
            ...
            |elem, engine, locator, styles, region| {
                ...
                let resolved_width: Option<TermScalar> = match elem.width.get(styles) {
                    ...
                    typst::foundations::Smart::Custom(rel) => {
                        Some(lynchpin_library::units::rel_to_cols(&rel, styles))
                    }
                };
                ...
                let resolved_height: Option<TermScalar> = match elem.height.get(styles) {
                    ...
                    typst::layout::Sizing::Rel(rel) => {
                        Some(lynchpin_library::units::rel_to_rows(&rel, styles))
                    }

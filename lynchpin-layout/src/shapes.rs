...
pub fn layout_curve(
    ...
    Ok(TermFrame::new(TermSize::new(
        lynchpin_library::resolve_page_size(styles).cols.min(TermScalar::new(10)),
        TermScalar::ONE,
    )))
}
...
fn layout_term_shape(
    ...
    // Determine size.
    let default_w: Col = lynchpin_library::resolve_page_size(styles).cols.min(TermScalar::new(20));

...
use lynchpin_library::config::TermConfig;
use lynchpin_library::frame::{Col, TermFrame, TermSize};
use tracing::debug;
...
impl<'cfg, 'eng, 'e> TermMathContext<'cfg, 'eng, 'e> {
    ...
    /// `lynchpin-layout`).  Instead it walks the content tree directly,
    ...
    if run.is_multiline() {
        ...
        if frame.cols() > Col::ZERO {
            let mut padded = TermFrame::new(TermSize::new(
                ...
                frame.rows().max(lynchpin_library::frame::Row::new(1)),
            ));
            padded.set_baseline(frame.baseline());
            padded.push_frame(lynchpin_library::frame::TermPoint::new(Col::new(1), lynchpin_library::frame::Row::ZERO), frame);

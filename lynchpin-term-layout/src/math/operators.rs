...
pub fn build_sum_operator(mode: RenderMode, _content_hint_rows: Row) -> TermFrame {
    ...
    for x in 0..width {
        sigma.push_text(TermPoint::new(x, 0), sc.top.to_string(), ContentStyle::default());
    }

    // Row 1: diag_top at col 0, mid char at col 1
    sigma.push_text(TermPoint::new(0, 1), sc.diag_top.to_string(), ContentStyle::default());
    // Row 2: diag_bot at col 0
    sigma.push_text(TermPoint::new(0, 2), sc.diag_bot.to_string(), ContentStyle::default());

    // Row 3: bottom bar (all summation_bottom)
    for x in 0..width {
        sigma.push_text(TermPoint::new(x, 3), sc.bot.to_string(), ContentStyle::default());
    }
...
pub fn build_prod_operator(mode: RenderMode) -> TermFrame {
    ...
    // Row 0: top bar
    for x in 0..width {
        prod.push_text(TermPoint::new(x, 0), pc.top.to_string(), ContentStyle::default());
    }
    // Rows 1-2: vertical bars at edges
    for y in 1..3 {
        prod.push_text(TermPoint::new(0, y), pc.vert.to_string(), ContentStyle::default());
        prod.push_text(TermPoint::new(2, y), pc.vert.to_string(), ContentStyle::default());
    }
    // Row 3: bottom bar
    for x in 0..width {
        prod.push_text(TermPoint::new(x, 3), pc.base.to_string(), ContentStyle::default());
    }
...
pub fn build_integral_operator(mode: RenderMode, content_rows: Row) -> TermFrame {
    ...
    let min_height: Row = if mode.is_unicode() { 2 } else { 3 };
    ...
    if mode.is_unicode() {
        ...
        integral.push_text(
            TermPoint::new(0, 0), ic.top.to_string(), ContentStyle::default(),
        );
        for y in 1..(height - 1) {
            integral.push_text(
                TermPoint::new(0, y), ic.mid.to_string(), ContentStyle::default(),
            );
        }
        if height > 1 {
            integral.push_text(
                TermPoint::new(0, height - 1), ic.bot.to_string(), ContentStyle::default(),
            );
        }
    } else {
        ...
        for (i, &ch) in top_chars.iter().enumerate() {
            integral.push_text(
                TermPoint::new(i as Col, 0), ch.to_string(), ContentStyle::default(),
            );
        }
        for y in 1..(height - 1) {
            for (i, &ch) in mid_chars.iter().enumerate() {
                integral.push_text(
                    TermPoint::new(i as Col, y), ch.to_string(), ContentStyle::default(),
                );
            }
        }
        if height > 1 {
...
fn sum_chars(mode: RenderMode) -> SumChars {
    if mode.is_unicode() {
        SumChars { top: '─', diag_top: '╲', diag_bot: '╱', bot: '‾' }
    } else {
        SumChars { top: '=', diag_top: '\\', diag_bot: '/', bot: '=' }
    }
}

...
use typst_library::layout::{
    Abs, Axes, BoxElem, Dir, Fr, HElem, InlineElem, InlineItem, Sizing, Spacing,
};
...
use super::*;
// NOTE: FrameModifiers, FrameModify, layout_and_modify are expected from crate::modifiers (Agent F)
...
pub enum Item<'a> {
    ...
    Fractional(
        Fr,
        Option<(&'a Packed<BoxElem>, Locator<'a>, StyleChain<'a>)>,
    ),
...
impl<'a> Item<'a> {
    ...
    pub fn natural_width(&self) -> Abs {
        match self {
            ...
            Self::Absolute(v, _) => *v,
            Self::Frame(frame) => Abs::raw(frame.cols().get() as f64),
        ...
        if child.is::<SpaceElem>() {
            ...
            collector.push_item(match elem.amount {
                ...
                Spacing::Rel(rel) => Item::Absolute(
                    rel.resolve(styles).relative_to(Abs::raw(region.cols.get() as f64)),
            ...
            // Convert TermSize to Axes<Abs> for typst's InlineElem::layout
            let region_axes = Axes::new(
                Abs::raw(region.cols.get() as f64),
                Abs::raw(region.rows.get() as f64),
            );

            for item in elem.layout(engine, locator.next(&elem.span()), styles, region_axes)? {
                match item {
                    InlineItem::Space(space, weak) => {
                        collector.push_item(Item::Absolute(space, weak));
                    }
                    InlineItem::Frame(frame) => {
                        // NOTE: FrameModifiers::get_in and FrameModify::modify
                        // are expected from crate::modifiers (Agent F).
                        // For now convert typst Frame to TermFrame directly.
                        // frame.modify(&FrameModifiers::get_in(styles));
                        // apply_shift expects &mut TermFrame, but we have typst Frame.
                        // Convert via creating a TermFrame and copying size.
            ...
            if let Sizing::Fr(v) = elem.width.get(styles) {
                ...
                let mut frame = layout_box(elem, engine, loc, styles, region)?;
